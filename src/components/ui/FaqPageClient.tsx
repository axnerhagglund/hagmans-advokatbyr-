        ...
        <div className="max-w-4xl mx-auto px-6">
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              <p className="font-display text-3xl text-[#888] mb-4">Hittade inget.</p>
              <p className="text-[#6b6b6b]">Prova ett annat sökord eller välj en annan kategori.</p>
            </div>
          ) : (
            <div className="border-2 border-[#0a0a0a]" style={{ boxShadow: "4px 4px 0 #0a0a0a" }}>
              {filtered.map((item, i) => (
                <div key={item._id} className={i > 0 ? "border-t-2 border-[#0a0a0a]" : ""}>
                  <button
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <span className="font-semibold text-sm leading-relaxed">{item.question}</span>
                    </div>
                    <span
                      ...
                      style={{ transform: openId === item._id ? "rotate(45deg)" : "none" }}
                    >
                      +
                    </span>
                  </button>
          ...
          <div className="mt-12 bg-[#ffeb3b] border-2 border-[#0a0a0a] p-6" style={{ boxShadow: "4px 4px 0 #0a0a0a" }}>
            <h3 className="font-display text-xl mb-2">Hittade du inte svaret du sökte?</h3>
            <p className="text-sm text-[#444] mb-4">
              Boka ett kostnadsfritt samtal och ställ din fråga direkt till en av våra jurister.
            </p>
