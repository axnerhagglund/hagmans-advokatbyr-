...
export const getAllTeamMembers = cache(async (): Promise<SanityTeamMember[]> => {
  ...
  return sanityClient.fetch(
    `*[_type == "teamMember"] | order(order asc) {
      _id, _type, name, title, specialties,
      bio, personal, email, phone, initials, accentColor, order
    }`,
    {},
    { next: { tags: ["team"] } }
  );
...
export const getAllServices = cache(async (): Promise<SanityService[]> => {
  ...
  return sanityClient.fetch(
    `*[_type == "service"] | order(order asc) {
      _id, _type, title, icon, shortDesc,
      whatItMeans, whenYouNeedIt, howWeHelp,
      price, color, order
    }`,
    {},
    { next: { tags: ["services"] } }
...
export const getAllPricingPackages = cache(async (): Promise<SanityPricingPackage[]> => {
  ...
  return sanityClient.fetch(
    `*[_type == "pricingPackage"] | order(order asc) {
      _id, _type, title, subtitle, price,
      priceNote, included, highlight, order
    }`,
    {},
    { next: { tags: ["pricing"] } }
  );
